...
class DiceItem {
  ...
  String get name => DiceNames.loreName(material, enchantment);

  /// Short label for compact displays
  String get shortName {
    final matName = DiceTiers.materialName(material);
    if (enchantment != null) {
      return '${DiceTiers.enchantmentName(enchantment!)} $matName';
    }
    return '$matName Die';
  }
...
class NpcData {
  ...
  static List<DiceItem> _generateNpcDice(NpcType type, int gold) {
    ...
    switch (type) {
      case NpcType.noble:
        // 2 epic, 2 rare, 2 uncommon — nobles are loaded
        return [
          _randomDieOfTier(DiceTier.epic, rng, 0.3),
          _randomDieOfTier(DiceTier.epic, rng, 0.25),
          _randomDieOfTier(DiceTier.rare, rng, 0.2),
          _randomDieOfTier(DiceTier.rare, rng, 0.15),
          _randomDieOfTier(DiceTier.uncommon, rng, 0.1),
          _randomDieOfTier(DiceTier.uncommon, rng, 0.1),
        ];
      case NpcType.merchant:
        ...
        return [
          _randomDieOfTier(DiceTier.rare, rng, 0.2),
          _randomDieOfTier(DiceTier.uncommon, rng, 0.12),
          _randomDieOfTier(DiceTier.uncommon, rng, 0.12),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
        ];
      case NpcType.blacksmith:
        // 2 rare (iron/bronze), 4 common
        return [
          DiceItem(material: DiceMaterial.iron),
          DiceItem(material: DiceMaterial.bronze, enchantment: rng.nextDouble() < 0.15 ? Enchantment.ironclad : null),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
          _randomDieOfTier(DiceTier.common, rng, 0.05),
      ...
      case NpcType.guard:
        // 1 uncommon, 5 common
        return [
          _randomDieOfTier(DiceTier.uncommon, rng, 0.08),
          _randomDieOfTier(DiceTier.common, rng, 0.03),
          _randomDieOfTier(DiceTier.common, rng, 0.03),
          _randomDieOfTier(DiceTier.common, rng, 0.03),
          _randomDieOfTier(DiceTier.common, rng, 0.03),
          _randomDieOfTier(DiceTier.common, rng, 0.03),
      ...
      default:
        // Farmers, villagers, barmaids — all common wood
        return List.generate(6, (_) {
          final mats = [DiceMaterial.birch, DiceMaterial.oak, DiceMaterial.mahogany];
          return DiceItem(material: mats[rng.nextInt(mats.length)]);
        });
    }
  }

  static DiceItem _randomDieOfTier(DiceTier tier, Random rng, double enchantChance) {
    final materials = DiceMaterial.values.where((m) => DiceTiers.tierOf(m) == tier).toList();
    final mat = materials[rng.nextInt(materials.length)];
    Enchantment? ench;
    if (rng.nextDouble() < enchantChance) {
      final available = Enchantment.values.where((e) =>
        DiceTiers.enchantmentMinTier(e).index <= tier.index).toList();
      if (available.isNotEmpty) ench = available[rng.nextInt(available.length)];
    }
...
class DiceTableData {
  ...
  /// Seat at the nearest chair based on entity position.
  bool seatNearest(String entityId, double entityX, double entityY) {
    if (isFull) return false;
    if (isEmpty) {
      // Pick the closer seat
      final seat0 = seatWorldPos(0);
      final seat1 = seatWorldPos(1);
      final d0 = (entityX - seat0.dx) * (entityX - seat0.dx) + (entityY - seat0.dy) * (entityY - seat0.dy);
      final d1 = (entityX - seat1.dx) * (entityX - seat1.dx) + (entityY - seat1.dy) * (entityY - seat1.dy);
      if (d0 <= d1) {
        occupant1Id = entityId;
      } else {
        occupant2Id = entityId;
  ...
  int npcScore1 = 0;
  int npcScore2 = 0;
  FarkleGameState? liveGameState; // persists actual game for observation
  ...
  void startNpcGame(int bet, double duration) {
    ...
    npcScore1 = 0;
    npcScore2 = 0;
    liveGameState = null; // created on first observation
  }
...
class FarkleGameState {
  ...
  static int _calcTargetScore(int bet) {
    if (bet <= 0) return GameConstants.farkleWinScore; // tournament = 4000
    switch (bet) {
      case 1:  return 1000;
      case 2:  return 1500;
      case 3:  return 2000;
      case 4:  return 2500;
      case 5:  return 3000;
      default: return GameConstants.farkleWinScore; // 6g+ = 4000
    }
  }

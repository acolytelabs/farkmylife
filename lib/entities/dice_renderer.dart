...
class MaterialDie extends StatelessWidget {
  ...
  final bool enchantmentOpportunity;
  ...
  const MaterialDie({
    ...
    this.enchantmentOpportunity = false,
  });
          ...
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                ...
                // Enchantment opportunity — bright pulsing glow when buff can trigger
                if (enchantmentOpportunity && !isScored && !isInactive)
                  BoxShadow(
                    color: EnchantmentColors.forEnchantment(ench ?? Enchantment.ironclad)
                        .withOpacity(0.4 + 0.3 * sin(gameTime * 4.0)),
                    blurRadius: 12 + 4 * sin(gameTime * 4.0),
                    spreadRadius: 2,
                  ),
  ...
  Color _borderColorFor(DiceMaterial mat, bool scored, bool selected) {
    ...
    // Border matches the die's material color family (darker shade)
    switch (mat) {
      case DiceMaterial.birch:    return const Color(0xFFa89870); // warm cream edge
      case DiceMaterial.oak:      return const Color(0xFF7a6038); // mid-brown edge
      case DiceMaterial.mahogany: return const Color(0xFF4a2210); // dark reddish edge
      case DiceMaterial.bone:     return const Color(0xFFb8a890); // warm bone edge
      case DiceMaterial.stone:    return const Color(0xFF606058); // cool gray edge
      case DiceMaterial.iron:     return const Color(0xFF4a5058); // dark steel edge
      case DiceMaterial.bronze:   return const Color(0xFF6a4818); // dark bronze edge
      case DiceMaterial.gold:     return const Color(0xFF8a6a10); // deep gold edge
      case DiceMaterial.jade:     return const Color(0xFF1a5a28); // dark jade edge
      case DiceMaterial.crystal:  return const Color(0xFF5080a0); // icy blue edge
      case DiceMaterial.obsidian: return const Color(0xFF4a2860); // dark purple edge
    }
  }
...
class MaterialDiePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 4. Enchantment: subtle particle sparkle effect (no corner badge)
    if (enchantment != null) {
      _paintEnchantmentParticles(canvas, size);
    }
  }
  ...
  void _paintEnchantmentParticles(Canvas canvas, Size size) {
    if (enchantment == null) return;
    final color = EnchantmentColors.forEnchantment(enchantment!);
    final t = gameTime;

    // 3-4 tiny sparkle particles that drift around the die edges
    for (int i = 0; i < 4; i++) {
      final phase = t * 0.8 + i * 1.57; // staggered
      final edge = (i % 4); // 0=top, 1=right, 2=bottom, 3=left
      double px, py;
      final slide = (sin(phase) + 1) / 2; // 0..1
      switch (edge) {
        case 0: px = size.width * slide; py = -1 + sin(phase * 2) * 2; break;
        case 1: px = size.width + 1 + sin(phase * 2) * 2; py = size.height * slide; break;
        case 2: px = size.width * (1 - slide); py = size.height + 1 + sin(phase * 2) * 2; break;
        default: px = -1 + sin(phase * 2) * 2; py = size.height * (1 - slide); break;
      }
      final alpha = ((sin(phase * 1.5) + 1) / 2 * 0.7).clamp(0.0, 1.0);
      canvas.drawCircle(Offset(px, py), 1.2,
        Paint()..color = color.withOpacity(alpha));
      // Soft glow around particle
      canvas.drawCircle(Offset(px, py), 3.0,
        Paint()..color = color.withOpacity(alpha * 0.2)
               ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2));
    }
  }
...
class MiniMaterialDiePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Enchantment: subtle outer glow instead of badge
    if (enchantment != null) {
      final enchColor = EnchantmentColors.forEnchantment(enchantment!);
      final glowP = Paint()
        ..color = enchColor.withOpacity(0.25)
        ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
      canvas.drawRRect(
    ...
    // Material-family border
    final borderP = Paint()
      ..color = _materialBorderColor().withOpacity(0.6)
  ...
  Color _materialBorderColor() {
    switch (material) {
      case DiceMaterial.birch:    return const Color(0xFFa89870);
      case DiceMaterial.oak:      return const Color(0xFF7a6038);
      case DiceMaterial.mahogany: return const Color(0xFF4a2210);
      case DiceMaterial.bone:     return const Color(0xFFb8a890);
      case DiceMaterial.stone:    return const Color(0xFF606058);
      case DiceMaterial.iron:     return const Color(0xFF4a5058);
      case DiceMaterial.bronze:   return const Color(0xFF6a4818);
      case DiceMaterial.gold:     return const Color(0xFF8a6a10);
      case DiceMaterial.jade:     return const Color(0xFF1a5a28);
      case DiceMaterial.crystal:  return const Color(0xFF5080a0);
      case DiceMaterial.obsidian: return const Color(0xFF4a2860);
    }
  }

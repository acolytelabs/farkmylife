...
class _GameShellState extends State<GameShell> with SingleTickerProviderStateMixin {
  ...
  void _onShellTick() {
    ...
    if (_showingBracketPreview) {
      _bracketPreviewTimer += dt;
      if (_bracketPreviewTimer >= 5.0 && _pendingTournamentOpponent != null) {
        _showingBracketPreview = false;
        setState(() {
          dialogueNpc = _pendingTournamentOpponent;
          farkleBet = 0;
          currentScreen = GameScreen.farkle;
        });
        _pendingTournamentMatch = null;
        _pendingTournamentOpponent = null;
        return;
      }
      if (mounted) setState(() {}); // refresh countdown
    }
    // Observe prompt auto-skip after 10 seconds
    if (_showObservePrompt && !_observePromptResolved) {
      _observePromptTimer += dt;
      if (_observePromptTimer >= 10.0) {
        _resolveObserve(false); // auto-skip
      }
      if (mounted) setState(() {});
    }
    // Always tick tournament regardless of what screen we're on
  ...
  NpcData? _pendingTournamentOpponent;
  double _bracketPreviewTimer = 0;
  bool _showingBracketPreview = false;
  ...
  void _onTournamentPlayerMatch(TournamentMatch match) {
    ...
    // Show bracket preview for 5 seconds before starting the match
    _pendingTournamentMatch = match;
    _pendingTournamentOpponent = opponent;
    _bracketPreviewTimer = 0;
    _showingBracketPreview = true;
  ...
  void _initializeGame() {
    ...
    diceTables = [
      ...
      DiceTableData(id: 2, x: 33, y: 16), // market
      DiceTableData(id: 3, x: 16, y: 29), // beer garden left
      DiceTableData(id: 4, x: 21, y: 29), // beer garden right
      DiceTableData(id: 5, x: 33, y: 21), // blacksmith courtyard
      DiceTableData(id: 6, x: 22, y: 33), // south park
    ...
    _tournament.onNpcMatchPrompt = _onTournamentNpcMatch;
  }

  // Tournament NPC match — show observe/skip prompt
  TournamentMatch? _pendingNpcMatch;
  bool _showObservePrompt = false;
  double _observePromptTimer = 0;
  bool _observePromptResolved = false;

  void _onTournamentNpcMatch(TournamentMatch match) {
    _pendingNpcMatch = match;
    _showObservePrompt = true;
    _observePromptTimer = 0;
    _observePromptResolved = false;
    setState(() {});
  }

  void _onTournamentObserveEnd(bool npc1Won) {
    if (_pendingNpcMatch != null) {
      final m = _pendingNpcMatch!;
      final winnerId = npc1Won ? m.entrant1Id : m.entrant2Id;
      m.winnerId = winnerId;
      m.score1 = npc1Won ? 4000 : 3000 + Random().nextInt(800);
      m.score2 = npc1Won ? 3000 + Random().nextInt(800) : 4000;
      m.completed = true;
      _showToast('${_tournament.entrantNames[_tournament.entrantIds.indexOf(winnerId ?? '')]} advances!',
          color: GameColors.uiHighlight);
    }
    _pendingNpcMatch = null;
    _observeNpc1 = null;
    _observeNpc2 = null;
    setState(() => currentScreen = GameScreen.overworld);
  }

  void _resolveObserve(bool observe) {
    if (_observePromptResolved || !_showObservePrompt) return;
    _observePromptResolved = true;
    _showObservePrompt = false;
    if (observe && _pendingNpcMatch != null) {
      final m = _pendingNpcMatch!;
      NpcData? npc1, npc2;
      for (final npc in npcs) {
        if (npc.id == m.entrant1Id) npc1 = npc;
        if (npc.id == m.entrant2Id) npc2 = npc;
      }
      if (npc1 != null && npc2 != null) {
        ...
        _observeBet = 0;
        _observeTable = null;
        _tournament.resolveObserveChoice(false);
        setState(() => currentScreen = GameScreen.observing);
        return;
      }
    }
    _tournament.resolveObserveChoice(false);
    _pendingNpcMatch = null;
    setState(() {}); // force UI refresh to dismiss modal
  }
  ...
  void _onObserveEnd(bool npc1Won, DiceItem? _) {
    // Check if this was a tournament observation
    if (_pendingNpcMatch != null) {
      _onTournamentObserveEnd(npc1Won);
      return;
    }
    setState(() {
  ...
  /// Called immediately when a game result is determined (gold exchange).
  void _onGameResult(bool playerWon) {
    setState(() {
      if (playerWon) {
      ...
      if (dialogueNpc != null) {
        player.history.recordGame(GameRecord(
          ...
          won: playerWon, betAmount: farkleBet,
          playerScore: 0, opponentScore: 0,
        ));
        // Record to NPC's history too
        dialogueNpc!.npcHistory.recordGame(GameRecord(
          opponentId: 'player',
          opponentName: 'Player',
          won: !playerWon, betAmount: farkleBet,
          playerScore: 0, opponentScore: 0,
  ...
  /// Called when player leaves the table (after rematch prompt resolved).
  void _onFarkleEnd(bool playerWon, DiceItem? loot) {
    ...
    setState(() {
      if (wasTournamentMatch) {
        ...
        if (playerWon) {
          _showToast('Tournament match won!', color: GameColors.uiSuccess);
        }
      ...
      // Release NPC from table — now they walk home
      if (dialogueNpc != null) {
        for (final t in diceTables) {
        ...
        dialogueNpc!.state = NpcState.returning;
    ...
    return Scaffold(
      body: Focus(
        ...
        onKeyEvent: (node, event) {
          if (_showObservePrompt && event is KeyDownEvent) {
            if (event.logicalKey == LogicalKeyboardKey.keyE) {
              _resolveObserve(true);
              return KeyEventResult.handled;
            }
            if (event.logicalKey == LogicalKeyboardKey.space ||
                event.logicalKey == LogicalKeyboardKey.escape) {
              _resolveObserve(false);
              return KeyEventResult.handled;
            }
            return KeyEventResult.handled; // block all keys during prompt
          }
        ...
        children: [
          ...
          // Persistent tournament banner during active phases
          if (_tournament.isActive &&
              currentScreen != GameScreen.farkle &&
              currentScreen != GameScreen.tournament &&
            ...
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: const Color(0xE0141008),
                  border: Border.all(color: GameColors.uiHighlight.withOpacity(0.6)),
                  borderRadius: BorderRadius.circular(4)),
                child: Text(_tournamentBannerText(),
                  textAlign: TextAlign.center,
                  style: const TextStyle(fontFamily: 'serif', fontSize: 10,
                    fontWeight: FontWeight.bold, color: GameColors.uiHighlight)),
              ),
            ),
          // Observe/Skip prompt for NPC tournament matches
          if (_showObservePrompt && _pendingNpcMatch != null)
            Positioned.fill(child: GestureDetector(
              ...
              child: Center(child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: GameColors.uiHighlight, width: 2),
                  borderRadius: BorderRadius.circular(8)),
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  const Text('⚔ TOURNAMENT MATCH', style: TextStyle(fontFamily: 'serif', fontSize: 14,
                    fontWeight: FontWeight.bold, color: GameColors.uiHighlight, letterSpacing: 2)),
                  const SizedBox(height: 8),
                  Builder(builder: (ctx) {
                    final ids = _tournament.entrantIds;
                    final names = _tournament.entrantNames;
                    final i1 = ids.indexOf(_pendingNpcMatch!.entrant1Id ?? '');
                    final i2 = ids.indexOf(_pendingNpcMatch!.entrant2Id ?? '');
                    final n1 = i1 >= 0 ? names[i1] : '?';
                    final n2 = i2 >= 0 ? names[i2] : '?';
                    return Text('${n1.toUpperCase()}  vs  ${n2.toUpperCase()}',
                      style: const TextStyle(fontFamily: 'monospace', fontSize: 12, color: GameColors.uiText));
                  }),
                  const SizedBox(height: 12),
                  // Countdown
                  Text('Auto-skip in ${(10.0 - _observePromptTimer).ceil()}s',
                    style: const TextStyle(fontFamily: 'monospace', fontSize: 9, color: GameColors.uiMuted)),
                  const SizedBox(height: 8),
                  Row(mainAxisSize: MainAxisSize.min, children: [
                    GestureDetector(
                      onTap: () => _resolveObserve(true),
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(4)),
                        child: const Text('[E] OBSERVE', style: TextStyle(fontFamily: 'monospace', fontSize: 11,
                          fontWeight: FontWeight.bold, color: Color(0xFF88ff88))))),
                    const SizedBox(width: 12),
                    GestureDetector(
                      onTap: () => _resolveObserve(false),
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                        decoration: BoxDecoration(
                          border: Border.all(color: const Color(0xFF5a4a28)),
                          borderRadius: BorderRadius.circular(4)),
                        child: const Text('[SPC] SKIP', style: TextStyle(fontFamily: 'monospace', fontSize: 11,
    ...
    switch (currentScreen) {
      ...
      case GameScreen.overworld:
        return OverworldWidget(
          ...
          onInteractTable: (table) {
            ...
            if (table.isEmpty) {
              // Player sits at nearest chair
              table.seatNearest('player', player.x, player.y);
          ...
          onNpcGameResolved: (winner, loser, bet) {
            ...
            _tournament.onGameCompleted();
            _shop.onGameCompleted();
            setState(() {}); // refresh UI to show tournament countdown
          },
          onInteractTournamentBoard: () {
            setState(() => currentScreen = GameScreen.tournament);
          },
          championName: _tournament.lastRecord?.winnerName,
          championId: _tournament.lastRecord?.winnerId,
          onCycleLoadout: () {
      ...
      case GameScreen.farkle:
        if (dialogueNpc != null) {
          ...
          return FarkleScreen(
            ...
            onGameResult: (playerWon) => _onGameResult(playerWon),
      ...
      case GameScreen.observing:
        if (_observeNpc1 != null && _observeNpc2 != null) {
          ...
          return FarkleScreen(
            ...
            observeScore1: _observeTable?.npcScore1 ?? 0,
            observeScore2: _observeTable?.npcScore2 ?? 0,
            observeTable: _observeTable,
          );
        }
      ...
      case GameScreen.tournament:
        return TournamentScreen(
          ...
          player: player,
          onClose: _showingBracketPreview ? null : () => _changeScreen(GameScreen.overworld),
          onRegister: () {
          ...
          getNpcs: () => npcs,
          previewCountdown: _showingBracketPreview
              ? (5.0 - _bracketPreviewTimer).clamp(0, 5).ceil() : null,
          previewOpponent: _pendingTournamentOpponent?.name,
        );
  ...
  String _tournamentBannerText() {
    final t = _tournament;
    switch (t.phase) {
      case TournamentPhase.announcing:
        return '⚔ ${t.tournamentName} — ANNOUNCING! Prize: ${t.prizeDie?.name ?? "?"}';
      case TournamentPhase.registration:
        final remaining = (TournamentManager.registrationDuration - t.phaseTimer).ceil();
        final mins = remaining ~/ 60;
        final secs = remaining % 60;
        final entrants = t.entrantIds.length;
        final playerIn = t.playerRegistered ? ' (YOU\'RE IN)' : ' — [E] at Board to enter!';
        return '⚔ REGISTRATION ${mins}:${secs.toString().padLeft(2, "0")} — $entrants/8 entered$playerIn';
      case TournamentPhase.bracketSet:
        return '⚔ ${t.tournamentName} — Bracket set!';
      case TournamentPhase.round1:
        return '⚔ ${t.tournamentName} — QUARTERFINALS in progress';
      case TournamentPhase.round2:
        return '⚔ ${t.tournamentName} — SEMIFINALS in progress';
      case TournamentPhase.finalRound:
        return '⚔ ${t.tournamentName} — THE FINAL!';
      case TournamentPhase.awards:
        return '⚔ ${t.tournamentName} — Champion crowned!';
      default:
        return '';
    }
  }
    ...
    return Transform.translate(
      ...
      child: Column(children: [
        ...
        Text('FARK MY LIFE', style: TextStyle(
          fontFamily: 'serif', fontSize: 44, fontWeight: FontWeight.w900,

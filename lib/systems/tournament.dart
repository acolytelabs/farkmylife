...
class TournamentManager {
  static const int entryFee = 1;
  static const int gamesToTrigger = 10; // NPC+player games
  static const double announceDuration = 30.0;
  static const double registrationDuration = 300.0; // 5 minutes
  static const double npcMatchDuration = 4.0;
  ...
  double phaseTimer = 0;
  int gameCounter = 0; // total games (NPC + player) since last tournament
  double idleTimer = 0;

  // ── Active tournament data ─────────────────────────────────────────
  List<String> entrantIds = [];
  List<String> entrantNames = [];
  ...
  // ═════════════════════════════════════════════════════════════════════
  // GAME COUNTER (NPC games + player games both count)
  // ═════════════════════════════════════════════════════════════════════
  ...
  bool _shouldStartTournament() {
    return phase == TournamentPhase.idle && gameCounter >= gamesToTrigger;
  }

  int get gamesUntilNext => phase == TournamentPhase.idle
      ? (gamesToTrigger - gameCounter).clamp(0, gamesToTrigger) : 0;
  ...
  bool tick(double dt) {
    ...
    switch (phase) {
      ...
      case TournamentPhase.registration:
        // NPCs trickle in during registration
        _tickNpcRegistration(dt);
  ...
  void _startAnnouncing() {
    ...
    prizeDie = _claimNextPrize();

    final prizeName = prizeDie!.name;
    onToast?.call('$tournamentName — Prize: $prizeName', GameColors.uiHighlight);
    GameAudio.tournamentHorn(); // Royal fanfare at announcement
  }

  void _startRegistration() {
    phase = TournamentPhase.registration;
    phaseTimer = 0;
    // Don't auto-register NPCs all at once — they trickle in via _tickNpcRegistration
    onToast?.call('Registration open for 5 minutes! Enter the Lists!', GameColors.uiHighlight);
    GameAudio.tournamentHorn();
  }

  double _npcRegTimer = 0;

  /// NPCs gradually register during the 5-min window.
  void _tickNpcRegistration(double dt) {
    _npcRegTimer += dt;
    // One NPC considers joining every ~20 seconds
    if (_npcRegTimer < 20.0) return;
    _npcRegTimer = 0;

    final npcs = getNpcs?.call() ?? [];
    // Always leave 1 seat for player until last 30 seconds
    final reservePlayerSeat = !playerRegistered && phaseTimer < (registrationDuration - 30.0);
    final maxNpcs = reservePlayerSeat ? 7 : 8;
    final currentNpcCount = entrantIds.where((id) => id != 'player').length;
    if (currentNpcCount >= maxNpcs) return;

    // Find eligible NPC not already registered
    final eligible = npcs.where((n) =>
        !entrantIds.contains(n.id) &&
        n.gold >= entryFee &&
    ...
    // Not everyone wants to join — 40% chance each tick
    final candidate = eligible[_rng.nextInt(eligible.length)];
    if (_rng.nextDouble() < 0.4) {
      candidate.gold -= entryFee;
      entrantIds.add(candidate.id);
      entrantNames.add(candidate.name);
      onToast?.call('${candidate.name} enters the lists!', null);
      GameAudio.entrantJoin();
    }
  }
  ...
  void _closeRegistration() {
    // Force-fill all remaining slots — everyone plays
    final npcs = getNpcs?.call() ?? [];
    final maxSlots = 8;
    ...
    for (final npc in eligible) {
      if (entrantIds.length >= maxSlots) break;
      // Charge fee if they can afford, waive if not
      if (npc.gold >= entryFee) npc.gold -= entryFee;
      entrantIds.add(npc.id);
      entrantNames.add(npc.name);
    }

    // If somehow still short, pad with byes
    while (entrantIds.length < maxSlots) {
  ...
  void Function(TournamentMatch match)? onNpcMatchPrompt;
  bool _waitingForObserveChoice = false;
  bool _observeChoiceMade = false;
  bool _playerWantsToObserve = false;

  /// Called by shell when player chooses observe or skip
  void resolveObserveChoice(bool observe) {
    _observeChoiceMade = true;
    _playerWantsToObserve = observe;
    _waitingForObserveChoice = false;
  }
  ...
  bool _tickMatches(double dt) {
    ...
    // NPC-vs-NPC: prompt player to observe or skip
    if (!_waitingForObserveChoice && !_observeChoiceMade) {
      _waitingForObserveChoice = true;
      onNpcMatchPrompt?.call(match);
      return false;
    }
    if (_waitingForObserveChoice) return false; // waiting for choice

    if (_observeChoiceMade && _playerWantsToObserve) {
      // Player wants to observe — hand off to shell like a player match
      _observeChoiceMade = false;
      _playerWantsToObserve = false;
      onPlayerMatchReady?.call(match); // reuse the same callback for observation
      return false;
    }

    // Player chose skip — simulate quickly
    _observeChoiceMade = false;
    _playerWantsToObserve = false;
  ...
  void _startAwards() {
    ...
    // Award prize
    GameAudio.tournamentChampion();
    final winRecord = TournamentWinRecord(
      tournamentName: tournamentName,
      prizeDieName: prizeDie?.name ?? 'Unknown',
      prizeTierIndex: prizeDie?.tier.index ?? 0,
    );

    if (winnerId == 'player' && prizeDie != null) {
      final p = getPlayer?.call();
      if (p != null) {
        p.dice.add(prizeDie!);
        p.rebuildDefaultLoadout();
        p.history.tournamentsWon++;
        p.history.tournamentWins.add(winRecord);
        p.history.earnedBadges.add(Badges.tourneyWin.id);
      }
      onToast?.call('YOU WIN THE TOURNAMENT! Prize: ${prizeDie!.name}', GameColors.tierLegendary);
    } else if (winnerId != null && prizeDie != null) {
      // NPC wins — add prize to their loadout + record tournament win
      final npcs = getNpcs?.call() ?? [];
      for (final npc in npcs) {
        if (npc.id == winnerId) {
          if (npc.npcDice.length >= 6) {
            int worstIdx = 0;
            int worstTier = 99;
            for (int i = 0; i < npc.npcDice.length; i++) {
              final t = DiceTiers.tierOf(npc.npcDice[i].material).index;
              if (t < worstTier) { worstTier = t; worstIdx = i; }
            }
            if (DiceTiers.tierOf(prizeDie!.material).index > worstTier) {
              npc.npcDice[worstIdx] = prizeDie!;
            }
          } else {
            npc.npcDice.add(prizeDie!);
          }
          // Record to NPC history
          npc.npcHistory.tournamentsWon++;
          npc.npcHistory.tournamentWins.add(winRecord);
          npc.npcHistory.earnedBadges.add(Badges.tourneyWin.id);
          break;
        }
      }
      onToast?.call('$winnerName wins the ${prizeDie!.name}!', GameColors.uiHighlight);
    }

    // Record tournament entry for all participants
    final allNpcs = getNpcs?.call() ?? [];
    for (final eid in entrantIds) {
      if (eid == 'player') {
        getPlayer?.call()?.history.tournamentsEntered++;
      } else {
        for (final npc in allNpcs) {
          if (npc.id == eid) { npc.npcHistory.tournamentsEntered++; break; }
        }
      }
    }
  ...
  void _finishTournament() {
    ...
    phaseTimer = 0;
    gameCounter = 0;
    idleTimer = 0; // reset hourly countdown
    _saveTournament();
  }
    ...
    for (final match in matches) {
      if (match.isPlayerMatch && !match.completed) {
        ...
        if (!playerWon) {
          ...
          GameAudio.tournamentEliminated();
        } else {
          GameAudio.matchStart(); // advance sound
        }
        break;
      }
    }
  ...
  // Upcoming tournament prizes (pre-generated for preview)
  List<DiceItem> upcomingPrizes = [];

  void ensureUpcomingPrizes() {
    while (upcomingPrizes.length < 3) {
      upcomingPrizes.add(_generatePrize());
    }
  }

  DiceItem _claimNextPrize() {
    ensureUpcomingPrizes();
    final prize = upcomingPrizes.removeAt(0);
    ensureUpcomingPrizes(); // refill
    return prize;
  }

  DiceItem _generatePrize() {
    // Tournament prizes are ALWAYS special — minimum Rare, always enchanted
    final tierRoll = _rng.nextDouble();
    DiceTier tier;
    if (tierRoll < 0.35)       tier = DiceTier.rare;
    else if (tierRoll < 0.70)  tier = DiceTier.epic;
    ...
    // Always enchanted for tournament prizes
    final ench = _pickEnchantment(tier);

...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget _buildLeftPanel() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: widget.portraitColor.withOpacity(0.3), blurRadius: 12)],
            ),
            child: CustomPaint(painter: _ProfilePortraitPainter(color: widget.portraitColor)),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Name:', style: _labelStyle()),
            Text(widget.name, style: TextStyle(fontFamily: 'monospace', fontSize: 18,
              fontWeight: FontWeight.bold, color: GameColors.uiHighlight)),
            const SizedBox(height: 4),
            Text(widget.title, style: TextStyle(fontFamily: 'monospace', fontSize: 10,
              color: const Color(0xFF8a7a50), fontWeight: FontWeight.w600, letterSpacing: 2)),
            const SizedBox(height: 4),
            Text('${widget.gold}g', style: const TextStyle(fontFamily: 'monospace', fontSize: 14,
        ...
        _statLine('Highest Turn Score', '${widget.history.highestSingleTurnScore}'),
        _statLine('Total Busts', '${widget.history.bustCount}'),
        _statLine('Hot Dice Rolled', '${widget.history.hotDiceCount}'),
        _statLine('Longest Win Streak', '${widget.history.longestWinStreak}'),
        _statLine('Net Gold', '${widget.history.netGold >= 0 ? "+" : ""}${widget.history.netGold}g'),
        _statLine('Tournaments Entered', '${widget.history.tournamentsEntered}'),
        _statLine('Tournaments Won', '${widget.history.tournamentsWon}'),

        // Tournament victories detail
        if (widget.history.tournamentWins.isNotEmpty) ...[
          const SizedBox(height: 6),
          Text('Tournament Victories:', style: TextStyle(fontFamily: 'serif', fontSize: 11,
            fontWeight: FontWeight.bold, color: GameColors.uiHighlight)),
          const SizedBox(height: 4),
          ...widget.history.tournamentWins.reversed.take(5).map((tw) => Padding(
            padding: const EdgeInsets.only(bottom: 2),
            child: Row(children: [
              const Text('🏆 ', style: TextStyle(fontSize: 9)),
              Expanded(child: Text(tw.tournamentName, style: const TextStyle(
                fontFamily: 'monospace', fontSize: 9, color: GameColors.uiText),
                overflow: TextOverflow.ellipsis)),
              Text(tw.prizeDieName, style: TextStyle(fontFamily: 'monospace', fontSize: 8,
                color: TierColors.forTier(DiceTier.values[tw.prizeTierIndex.clamp(0, 4)]))),
            ]),
          )),
        ],
        ...
        // ── Equipped Dice ──
        Text('Dice Loadout:', style: _headerStyle()),
        const SizedBox(height: 6),
        if (widget.equippedDice != null && widget.equippedDice!.isNotEmpty)
          Row(children: List.generate(min(6, widget.equippedDice!.length), (i) {
            final die = widget.equippedDice![i];
            final selected = _selectedDieIdx == i;
            return GestureDetector(
              onTap: () => setState(() => _selectedDieIdx = selected ? null : i),
              child: Container(
                width: 32, height: 32,
                margin: const EdgeInsets.only(right: 4),
                decoration: BoxDecoration(
                  border: Border.all(
                    color: selected ? GameColors.uiHighlight : TierColors.forTier(die.tier).withOpacity(0.5),
                    width: selected ? 2 : 1),
                  borderRadius: BorderRadius.circular(3),
                  color: const Color(0xFF0a0a08)),
                child: CustomPaint(painter: MiniMaterialDiePainter(
                  value: (i % 6) + 1, material: die.material, enchantment: die.enchantment)),
              ),
            );
          }))
        else
          Text('No dice', style: _valueStyle(color: GameColors.uiMuted)),
        // Selected die detail
        if (_selectedDieIdx != null && widget.equippedDice != null &&
            _selectedDieIdx! < widget.equippedDice!.length) ...[
          const SizedBox(height: 6),
          Container(
            padding: const EdgeInsets.all(6),
            decoration: BoxDecoration(
              color: const Color(0xFF0c0a14),
              border: Border.all(color: widget.equippedDice![_selectedDieIdx!].tierColor.withOpacity(0.3)),
              borderRadius: BorderRadius.circular(4)),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(widget.equippedDice![_selectedDieIdx!].name, style: TextStyle(
                fontFamily: 'monospace', fontSize: 10, fontWeight: FontWeight.bold,
                color: widget.equippedDice![_selectedDieIdx!].tierColor)),
              Text(widget.equippedDice![_selectedDieIdx!].description, style: const TextStyle(
                fontFamily: 'monospace', fontSize: 8, color: Color(0xFF8a7a60))),
            ]),
          ),
        ],
        if (widget.equippedDice != null)
          Text('${widget.diceCount} total dice owned',
            style: _valueStyle(color: GameColors.uiMuted)),
  ...
  Widget _buildStatTable() {
    ...
    final opponents = widget.history.opponents.values.toList()
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        ...
        SizedBox(width: 40, child: Text('${widget.history.totalWins}',
          style: _valueStyle(color: GameColors.uiSuccess))),
        SizedBox(width: 50, child: Text('${widget.history.totalLosses}',
          style: _valueStyle(color: widget.history.totalLosses > 0 ? GameColors.uiDanger : GameColors.uiText))),
        SizedBox(width: 50, child: Text('${(widget.history.winRate * 100).toStringAsFixed(1)}%',
          style: _valueStyle(color: widget.history.winRate >= 0.5 ? GameColors.uiSuccess : GameColors.uiDanger))),
      ]),
    ]);
  }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 6, runSpacing: 6,
          children: Badges.all.map((badge) {
            final earned = widget.history.earnedBadges.contains(badge.id);
            return Tooltip(
        ...
        const SizedBox(height: 6),
        Expanded(
          child: widget.history.recentGames.isEmpty
              ? Center(child: Text('No games yet.', style: _valueStyle(color: GameColors.uiMuted)))
              : ListView.builder(
                  itemCount: widget.history.recentGames.length,
                  itemBuilder: (ctx, i) {
                    // Show most recent first
                    final game = widget.history.recentGames[widget.history.recentGames.length - 1 - i];
                    return Container(

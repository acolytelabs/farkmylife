...
class FarkleScreen extends StatefulWidget {
  ...
  /// Called immediately when a game result is determined (for gold exchange).
  /// onGameEnd is called when the player actually leaves the table.
  final void Function(bool playerWon)? onGameResult;
  ...
  final int observeScore1;
  final int observeScore2;
  final DiceTableData? observeTable;
  ...
  const FarkleScreen({
    ...
    this.observeScore1 = 0,
    this.observeScore2 = 0,
    this.observeTable,
  });
...
class _FarkleScreenState extends State<FarkleScreen>
  ...
  bool _visualPlayerTurn = true; // tracks whose dice are visually on the felt
  int _bustLostPts = 0;
  ...
  void initState() {
    ...
    if (_isObserving) {
      final npc1Dice = widget.observeNpc1!.npcDice;
      final npc2Dice = widget.opponent.npcDice;
      _playerDice = List.generate(6, (i) => FarkleDie(
        item: i < npc1Dice.length ? npc1Dice[i] : DiceItem.wooden));
      _npcDice = List.generate(6, (i) => FarkleDie(
        item: i < npc2Dice.length ? npc2Dice[i] : DiceItem.wooden));

      // Resume from live game state if available
      final table = widget.observeTable;
      if (table != null && table.liveGameState != null) {
        _game = table.liveGameState!;
        _game.dice = _game.isPlayer1Turn ? _playerDice : _npcDice;
        // Restore dice values from the live state
      } else {
        ...
        _game.player1Score = widget.observeScore1;
        _game.player2Score = widget.observeScore2;
        // Store on table for future re-entry
        if (table != null) table.liveGameState = _game;
      }
      ...
      final npcEquipped = widget.opponent.npcDice;
      _npcDice = List.generate(6, (i) => FarkleDie(
        item: i < npcEquipped.length ? npcEquipped[i] : DiceItem.wooden,
      ));
      _game = FarkleGameState(
        player1Id: 'player',
        player2Id: widget.opponent.id,
    ...
    _generateScatter(6);
    _visualPlayerTurn = _game.isPlayer1Turn;
  ...
  void _onTick() {
    ...
    if (_showingTurnResult) {
      ...
      if (_resultTimer >= duration) {
        ...
        _resultTimer = 0;
        _npcThinking = false; // ALWAYS stop NPC thinking during turn transition
        // Apply deferred farkle NOW (after player has seen their dice)
        if (_pendingFarkle) {
          final wasTurnBefore = _game.isPlayer1Turn;
          FarkleEngine.applyFarkle(_game);
          _pendingFarkle = false;
          // Check if the turn actually switched (Second Wind prevents it)
          if (_game.isPlayer1Turn == wasTurnBefore && !_game.farkled) {
            // Second Wind triggered — same player rolls again, no swap
            _game.phase = FarklePhase.rolling;
            _diceInPlay = false;
            _startNextPhase();
            setState(() {});
    ...
    if (_npcThinking) {
      _npcTimer += dt;
      // Step 15 (dice selection) runs at its own faster rate
      if (_npcStep == 15) {
        _executeNpcStep();
      } else if (_npcTimer >= GameConstants.npcTurnDelay) {
        _npcTimer = 0;
        _executeNpcStep();
      }
      setState(() {});
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    ...
    // Observation mode: ESC or E to leave
    if (_isObserving) {
    ...
    // Game over — handle rematch prompt
    if (_game.phase == FarklePhase.gameOver) {
      if (_showRematchPrompt) {
        if (event.logicalKey == LogicalKeyboardKey.keyE && _rematchAvailable && !_rematchDeclined) {
          _acceptRematch();
          return KeyEventResult.handled;
        }
        if (event.logicalKey == LogicalKeyboardKey.escape ||
    ...
    if (key == LogicalKeyboardKey.keyE && _game.phase == FarklePhase.selecting) {
      if (_cursorIndex >= 0 && _cursorIndex < _game.dice.length) {
        ...
        if (!die.held && !die.locked) {
          if (die.selected || _isDieSelectable(_cursorIndex)) {
            die.selected = !die.selected; GameAudio.diceSelect(); setState(() {});
          }
          return KeyEventResult.handled;
        }
      }
    ...
    // ── Space: Roll (rolling phase) or Keep & Roll (selecting phase) ──
    if (key == LogicalKeyboardKey.space) {
      if (_game.phase == FarklePhase.rolling) {
        _rollDice();
        return KeyEventResult.handled;
      }
      if (_game.phase == FarklePhase.selecting) {
        // If dice already selected, keep & roll normally
        if (_game.selectedDice.isNotEmpty) {
          _keepAndRoll();
          return KeyEventResult.handled;
        }
        // If no dice selected, check if ALL available dice are scorable
        final available = _game.dice.where((d) => !d.held && !d.locked).toList();
        if (available.isNotEmpty) {
          final allScore = FarkleEngine.scoreAllAvailable(_game.dice);
          if (allScore.score > 0) {
            // Check if literally every available die contributes to scoring
            final availableValues = available.map((d) => d.value).toList();
            final counts = <int, int>{};
            for (final v in availableValues) counts[v] = (counts[v] ?? 0) + 1;
            bool allContribute = true;

            // Special case: exactly 6 dice — check straight and three pairs
            if (available.length == 6) {
              final sorted = List<int>.from(availableValues)..sort();
              final isStraight = sorted[0] == 1 && sorted[1] == 2 && sorted[2] == 3 &&
                  sorted[3] == 4 && sorted[4] == 5 && sorted[5] == 6;
              final isThreePairs = counts.length == 3 && counts.values.every((c) => c == 2);
              if (isStraight || isThreePairs) {
                for (final d in available) d.selected = true;
                GameAudio.diceSelect();
                _keepAndRoll();
                return KeyEventResult.handled;
              }
            }

            // Normal check: each die must be a 1, 5, or part of a triple+
            for (final d in available) {
              final v = d.value;
              if (v != 1 && v != 5 && (counts[v] ?? 0) < 3) {
                allContribute = false;
                break;
              }
            }
            if (allContribute) {
              // Select all and keep & roll
              for (final d in available) d.selected = true;
              GameAudio.diceSelect();
              _keepAndRoll();
              return KeyEventResult.handled;
  ...
  void _afterRoll() {
    ...
    _statusText = 'Your roll. [E] select, [SPC] keep & roll, [W] score';
    _game.phase = FarklePhase.selecting;
    setState(() {});
  }

  /// Check if a specific die can be part of a scoring combination.
  bool _isDieSelectable(int index) {
    final die = _game.dice[index];
    if (die.held || die.locked) return false;
    final v = die.value;
    // 1s and 5s are always scorable individually
    if (v == 1 || v == 5) return true;
    // Check if this die is part of a triple (or better)
    int count = 0;
    for (final d in _game.dice) {
      if (!d.held && !d.locked && d.value == v) count++;
    }
    if (count >= 3) return true;
    // Check if all 6 unheld dice form a straight
    final available = _game.dice.where((d) => !d.held && !d.locked).toList();
    if (available.length == 6) {
      final sorted = available.map((d) => d.value).toList()..sort();
      if (sorted[0] == 1 && sorted[1] == 2 && sorted[2] == 3 &&
          sorted[3] == 4 && sorted[4] == 5 && sorted[5] == 6) return true;
      // Three pairs
      final counts = <int, int>{};
      for (final d in available) counts[d.value] = (counts[d.value] ?? 0) + 1;
      if (counts.length == 3 && counts.values.every((c) => c == 2)) return true;
    }
    return false;
  }
  ...
  void _toggleDieSelection(int index) {
    ...
    // Allow deselecting always, but only select if die is scorable
    if (!die.selected && !_isDieSelectable(index)) return;
    setState(() {
  ...
  void _executeNpcStep() {
    if (!_npcThinking) return; // Safety: don't execute if we've already stopped
    if (_game.phase == FarklePhase.gameOver) { _npcThinking = false; return; }
    if (_showBustOverlay) return; // Don't advance during bust display
    ...
    switch (_npcStep) {
      ...
      case 15:
        ...
        _npcSelectionTimer += 1 / 60; // ~frame time

        // Determine delay: fast (0.08s) within a value group, slow (0.4s) between groups
        double delay = 0.5; // default pause between groups
        if (_npcSelectionIndex > 0 && _npcSelectionIndex < _npcSelectionQueue.length) {
          ...
          if (_game.dice[prevIdx].value == _game.dice[curIdx].value) {
            delay = 0.04; // same value = same group, select fast
          }
        }
  ...
  // Rematch state
  bool _showRematchPrompt = false;
  String _rematchMessage = '';
  bool _rematchAvailable = false;
  ...
  void _handleGameOver() {
    ...
    DiceItem? loot;
    final bet = widget.betAmount;
    final npcName = widget.opponent.name;
    if (playerWon) {
      ...
      _statusText = bet > 0
          ? '\u{1F451} VICTORY! You won ${bet}g from $npcName!'
          : '\u{1F451} VICTORY! You advance!';
      GameAudio.victory();
      if (bet > 0) GameAudio.goldClink();
    } else {
      ...
      _statusText = bet > 0
          ? '\u{1F480} DEFEAT. $npcName takes ${bet}g from you.'
          : '\u{1F480} DEFEAT. $npcName advances.';
      GameAudio.defeat();
    }
    _game.phase = FarklePhase.gameOver;
    _pendingLoot = loot;
    ...
    // Immediately handle gold exchange for this game
    widget.onGameResult?.call(playerWon);

    setState(() {});

    // After a brief delay, show rematch prompt (or auto-exit for tournaments)
    Future.delayed(const Duration(seconds: 2), () {
      if (!mounted) return;
      // No rematch in tournament matches — show results longer then exit
      if (widget.tournamentRound != null) {
        Future.delayed(const Duration(seconds: 5), () {
          if (mounted) widget.onGameEnd(_pendingPlayerWon, _pendingLoot);
        });
        return;
      }
      _determineRematch(playerWon);
      setState(() => _showRematchPrompt = true);
    });
  }

  DiceItem? _pendingLoot;
  bool _pendingPlayerWon = false;

  void _determineRematch(bool playerWon) {
    final npc = widget.opponent;
    final npcGold = npc.gold;
    final playerGold = widget.player.gold;
    final bet = widget.betAmount;

    if (playerWon) {
      // NPC might offer rematch (wants their gold back)
      if (npcGold < bet) {
        _rematchMessage = '"I\'m out of coin. Another time."';
        _rematchAvailable = false;
      } else if (_rng.nextDouble() < 0.6) {
        final offers = [
          '"Double or nothing! One more round?"',
          '"Lucky roll. Again?"',
          '"I demand a rematch!"',
          '"Best of three? Come on."',
          '"My dice were cold. Once more."',
        ];
        _rematchMessage = offers[_rng.nextInt(offers.length)];
        _rematchAvailable = true;
      } else {
        final declines = [
          '"Well played. I know when to quit."',
          '"I\'ll be back another day."',
          '"My pride can\'t take another loss right now."',
        ];
        _rematchMessage = declines[_rng.nextInt(declines.length)];
        _rematchAvailable = false;
      }
    } else {
      // Player lost — can they challenge a rematch?
      if (playerGold < bet) {
        _rematchMessage = 'You can\'t afford the wager.';
        _rematchAvailable = false;
      } else if (_rng.nextDouble() < 0.5) {
        _rematchMessage = '"Sure, I\'ll take more of your gold."';
        _rematchAvailable = true;
      } else {
        final refuses = [
          '"I\'ve had my fill. Find another mark."',
          '"Quit while I\'m ahead, I say."',
          '"Maybe after a drink."',
          '"I have business to attend to."',
        ];
        _rematchMessage = refuses[_rng.nextInt(refuses.length)];
        _rematchAvailable = false;
      }
    }
  ...
  void _acceptRematch() {
    // Reset game state for a new game, same bet
    ...
    for (final d in _playerDice) {
      d.held = false; d.selected = false; d.locked = false; d.value = 1;
    }
    for (final d in _npcDice) {
      d.held = false; d.selected = false; d.locked = false; d.value = 1;
    }
    _game.dice = _playerDice;
    _diceInPlay = false;
    ...
    _pendingFarkle = false;
    _pendingDiceSwap = false;
    _visualPlayerTurn = true;
    _npcStep = 0;
    _statusText = 'Rematch! Roll the dice! [SPACE]';
    _npcChatter = '"Let\'s go again!"';
    _generateScatter(6);
    setState(() {});
  }

  void _declineRematch() {
    widget.onGameEnd(_pendingPlayerWon, _pendingLoot);
  }
        ...
        child: SafeArea(
          child: Stack(
            children: [
              Column(
                children: [
                  _NpcBarWithChatter(
                    ...
                    rematchMessage: _showRematchPrompt ? _rematchMessage : null,
                    rematchAvailable: _rematchAvailable,
                    onRematch: _acceptRematch,
                    onLeave: _declineRematch,
                  ),
                  _TurnScoreDisplay(
                  ...
                  Expanded(
                    child: _FeltTable(
                      ...
                      inactiveDice: _visualPlayerTurn ? _npcDice : _playerDice,
                      rolling: _rolling,
                      ...
                      gameTime: _gameTime,
                      cursorIndex: (_game.isPlayer1Turn && !_isObserving && !_rolling && _game.phase == FarklePhase.selecting) ? _cursorIndex : -1,
                    ),
                  ),
                  const SizedBox(height: 4),
                  Builder(builder: (ctx) {
                    String text = _statusText;
                    bool isFarkle = _showBustOverlay;
                    // During player selecting phase, show dynamic score info
                    if (_game.phase == FarklePhase.selecting &&
                        _game.isPlayer1Turn && !_isObserving && !_rolling &&
                        !_showingTurnResult && _game.phase != FarklePhase.gameOver) {
                      final selScore = _game.selectedDice.isNotEmpty
                          ? (FarkleEngine.scoreSelectedDice(_game.dice)?.score ?? 0) : 0;
                      final bankTotal = _game.turnScore + selScore;
                      if (selScore > 0) {
                        text = 'SCORE $bankTotal [W]  •  KEEP & ROLL [SPC]  •  [E] select';
                      }
                    }
                    return _StatusBanner(text: text, isFarkle: isFarkle);
                  }),
                  const SizedBox(height: 6),
                  // (Action buttons removed — controls shown in status bar)
                  // Game over: observation mode auto-exits, player mode shows in status
                  // (Rematch controls moved to NPC bar)
                  const SizedBox(height: 6),
                  _HorizPlayerBar(
              ...
              // Observation overlay: "Watching" banner with small ESC hint
              if (_isObserving) ...[
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      GestureDetector(
                        ...
                        child: const Text('ESC', style: TextStyle(color: Color(0xFF6a5a40), fontSize: 8,
...
class _NpcBarWithChatter extends StatelessWidget {
  ...
  final String? tournamentRound;
  final String? rematchMessage;
  final bool rematchAvailable;
  ...
  const _NpcBarWithChatter({
    ...
    this.tournamentRound, this.rematchMessage, this.rematchAvailable = false,
    this.onRematch, this.onLeave,
  });
  @override
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (rematchMessage != null)
          Padding(padding: const EdgeInsets.only(left: 64, top: 2, bottom: 2),
            child: Row(children: [
              Expanded(child: Text(rematchMessage!, maxLines: 1, overflow: TextOverflow.ellipsis,
                style: TextStyle(fontFamily: 'monospace', fontSize: 9, fontStyle: FontStyle.italic,
                  color: rematchAvailable ? const Color(0xFFd0c8a0) : const Color(0xFF6a5a40)))),
              const SizedBox(width: 6),
              if (rematchAvailable)
                GestureDetector(onTap: onRematch,
                  child: Container(
                    ...
                    child: const Text('[E]', style: TextStyle(fontFamily: 'monospace', fontSize: 9,
                      fontWeight: FontWeight.bold, color: Color(0xFF88ff88))))),
              const SizedBox(width: 4),
              GestureDetector(onTap: onLeave,
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
                    border: Border.all(color: const Color(0xFF4a3a28)),
                    borderRadius: BorderRadius.circular(3)),
                  child: const Text('[ESC]', style: TextStyle(fontFamily: 'monospace', fontSize: 8,
...
class _FeltTable extends StatelessWidget {
        ...
        return Stack(children: [
          ...
          Positioned(right: 8, top: 8, bottom: 8, child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              // ── Selected die stats ──
              if (cursorIndex >= 0 && cursorIndex < activeDice.length &&
                  !rolling && diceInPlay) ...[
                Container(height: 1, color: const Color(0xFF8a6a38).withOpacity(0.2),
                  margin: const EdgeInsets.symmetric(vertical: 4)),
                Builder(builder: (ctx) {
                  final die = activeDice[cursorIndex];
                  final item = die.item;
                  final hasEnch = item.enchantment != null;
                  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(item.name, style: TextStyle(
                      fontFamily: hasEnch ? 'serif' : 'monospace', fontSize: 9,
                      fontWeight: FontWeight.bold, color: item.tierColor,
                      fontStyle: hasEnch ? FontStyle.italic : FontStyle.normal),
                      overflow: TextOverflow.ellipsis),
                    const SizedBox(height: 2),
                    Text(DiceTiers.materialStatDesc(item.material),
                      style: TextStyle(fontFamily: 'monospace', fontSize: 7,
                        color: item.tierColor.withOpacity(0.7))),
                    if (hasEnch) ...[
                      const SizedBox(height: 2),
                      Text('${DiceTiers.enchantmentName(item.enchantment!)}',
                        style: TextStyle(fontFamily: 'monospace', fontSize: 7,
                          fontWeight: FontWeight.bold,
                          color: EnchantmentColors.forEnchantment(item.enchantment!))),
                      Text(DiceTiers.enchantmentDesc(item.enchantment!),
          ...
          // (Bust overlay removed — indicated by status banner instead)
        ]);
      }),
    );
  }
  ...
  Widget _buildScatteredDie(int dieIdx, int scatterIdx,
    ...
    final die = activeDice[dieIdx];
    final hasOpp = _hasEnchantmentOpportunity(die, dieIdx);
    return Positioned(left: dx, top: dy, child: Transform.rotate(angle: rot,
      child: MaterialDie(
        die: die,
        rolling: rolling && !die.held && !die.locked,
        onTap: onTapDie != null && !die.held && !die.locked
            ? () => onTapDie!(dieIdx) : null,
        gameTime: gameTime,
        isCursorTarget: cursorIndex == dieIdx,
        enchantmentOpportunity: hasOpp,
      ),
    ));
  }

  /// Check if a die's enchantment has an active opportunity on the table.
  bool _hasEnchantmentOpportunity(FarkleDie die, int dieIdx) {
    if (die.held || die.locked || rolling) return false;
    final ench = die.item.enchantment;
    if (ench == null) return false;

    switch (ench) {
      case Enchantment.boneCollector:
        // Glow when this die is part of a triple
        int count = 0;
        for (final d in activeDice) {
          if (!d.held && !d.locked && d.value == die.value) count++;
        }
        return count >= 3;
      case Enchantment.gamblersEdge:
        // Glow on first scoring combo when turn score is 0
        // (indicated at the start of a turn before any scoring)
        return !die.held && !die.locked; // always glows to remind player
      case Enchantment.ironclad:
        // Glow when this die is part of any scoring combination
        final v = die.value;
        if (v == 1 || v == 5) return true;
        int count = 0;
        for (final d in activeDice) {
        ...
        return count >= 3;
      default:
        return false; // passive enchantments with no table-specific trigger
    }
  }
  ...
  Widget _buildHeldDie(int dieIdx, int stackIdx, double clampW, double clampH) {
    ...
    return Positioned(left: dx, top: dy,
      child: Transform.rotate(angle: heldRots[oi],
        child: Opacity(opacity: 1.0, child: MaterialDie(die: activeDice[dieIdx], rolling: false, onTap: null, isInactive: true, gameTime: gameTime)),
      ),
    );
  }
